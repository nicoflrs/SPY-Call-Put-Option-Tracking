...
/****** Object:  StoredProcedure [dbo].[sp_SPYPutData]    Script Date: 10/27/2021 1:29:44 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[sp_SPYPutData]
AS


--IF OBJECT_ID(N'money..[SPYPutData]') IS NOT NULL
--delete from money..[SPYPutData]
...
IF OBJECT_ID(N'tempdb..#putdata1') IS NOT NULL
DROP TABLE #putdata1

IF OBJECT_ID(N'tempdb..#putdata2') IS NOT NULL
DROP TABLE #putdata2

IF OBJECT_ID(N'tempdb..#putdata3') IS NOT NULL
DROP TABLE #putdata3
...
select [underlying].value as 'Ticker', [Put_Series_Date].[key] as 'ExpirationDate', [OptionData].[key] as 'Strike', [OptionData3].* 
into #APITemp1
FROM OPENJSON((SELECT * FROM @json))
WITH (putExpDateMap NVARCHAR(MAX) AS JSON, underlying NVARCHAR(MAX) AS JSON) AS  MetaData
CROSS APPLY OPENJSON([MetaData].[underlying]) AS underlying
CROSS APPLY OPENJSON([MetaData].[putExpDateMap]) AS Put_Series_Date
CROSS APPLY OPENJSON([Put_Series_Date].[value]) AS OptionData
...
insert into money.dbo.[SPYPutData]
select *
from #putdata3
